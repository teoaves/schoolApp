...
public class TeacherValidator<T> {
    ...
    public static<T extends BaseDTO> Map<String, String> validate(T dto) {
        ...
        if (dto.getFirstname().length() < 2 || dto.getLastname().length() > 32) {
            errors.put("firstname", "Το όνομα πρέπει να είναι μεταξύ 2-32 χαρακτήρων");
        }
        ...
        if(dto.getLastname().length() < 2 || dto.getFirstname().length() > 32) {
            errors.put("lastname", "Το επώνυμο πρέπει να είναι μεταξύ 2-32 χαρακτήρων");
